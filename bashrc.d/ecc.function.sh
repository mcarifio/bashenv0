...
#  by subshells.
function ecc {
  local me=$(realpath -s ${BASH_SOURCE:-$0})
  local here=$(dirname ${me})

  # name, the "thing" to edit
  local name=${1:?"${me}: expecting a name"}
  ...
  case "${name_type}" in
    ...
    file) pathame=$(type -p ${name}); [[ -z "${pathname}" ]] && _err "Source for script '${name}' not found. Stop." ;;
