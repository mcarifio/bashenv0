...
function _catch {
    ...
    declare -i _the_lineno="${_positionals[0]:-${LINENO}}"
    local _message="${_positionals[1]:-${FUNCNAME[0]}}"
    if (( ${_print} )) ; then
        _message ${_the_lineno} ${_message} ${_status} > ${_to}
...
# trap _on_exit EXIT
trap '_catch ${LINENO} default-catcher $?' EXIT
